...
if(test-path $publishpath){
    ...
    if(test-path $CoreClientProjectPath){
        $process = Start-Process -file "dotnet" -ArgumentList " publish $CoreClientProjectPath --output ""$publishPath\Agent\OSX"" --runtime osx-x64 --framework netcoreapp3.1" -Wait -PassThru
        write-host "OSX Exit code: $($process.ExitCode)"
        $process = Start-Process -file "dotnet" -ArgumentList " publish $CoreDaemonProjectPath --output ""$publishPath\Agent\ARM"" --runtime linux-arm --framework netcoreapp3.1" -Wait -PassThru
        write-host "ARM Exit code: $($process.ExitCode)"
        $process = Start-Process -file "dotnet" -ArgumentList " publish $CoreDaemonProjectPath --output ""$publishPath\Agent\Linux-x64"" --runtime linux-x64 --framework netcoreapp3.1" -Wait -PassThru
    ...
    if(test-path $WebServiceProjectPath){
        $process = start-process -filepath "dotnet" -ArgumentList " publish $WebServiceProjectPath --output ""$publishPath\WebService"" --runtime win-x86 --framework netcoreapp3.1 --self-contained" -Wait -PassThru
